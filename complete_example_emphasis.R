...
al = tan(1)
be = 0
pars = c(l0,ga,mu)*0.01
input = list(brts=brts_dendroica,pars=pars,sample_size=100,model="pd",importance_sampler="emphasis",cores=2,method="inverse")
...
for(i in 1:10000){
  ...
  pars = M$po$par
  save(MCEM,input,file="dendroica_pd_21Nov.RData")
}

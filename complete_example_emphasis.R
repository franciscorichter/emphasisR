...
for(i in 1:100){
  print(pars)
  time = proc.time()
  st1 = mc_sample_independent_trees(brts = brts_vangidae,pars = pars,nsim = input$sample_size,model = input$model, importance_sampler = input$importance_sampler,no_cores = input$cores,maxnumspec = input$maxspec, method = input$method)
  get.time(time)
 # print(log(st1$fhat))

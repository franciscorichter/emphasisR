...
pars = c(l0,ga,mu,0)
input = list(brts=brts_dendroica,pars=pars,sample_size=100,model="rpd5",importance_sampler="emphasis",cores=2,method="inverse",aceleration_rate=1.1)
#pars = c( -0.2505580,-0.9612669,0.5727150)

##  complete iterative framework 
MCEM = data.frame(par1=NULL,par2=NULL,par3=NULL,par4=NULL,loglik_hat=NULL,E_time=NULL,M_time=NULL)
...
for(i in 1:10000){
  ...
  MCEM = rbind(MCEM,data.frame(par1=pars[1],par2=pars[2],par3=pars[3],par4=pars[4],loglik_hat=log(st$fhat),E_time=st$E_time,M_time=M$M_time,sample_size=sample_size))
  pars = M$po$par

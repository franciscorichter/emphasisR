...
theme_emphasis =  theme(panel.grid.major = element_blank(), panel.grid.minor = element_blank(),
                        panel.background = element_blank(), axis.line = element_line(colour = "black"),axis.title=element_text(size=18,face="bold"))
                ...
                sidebarPanel(id="sidebar",
                             ...
                             selectInput("brts", "Choose Phylo/Branching times:",
                                         list("Other" = 0,
                                              ...
                                              "Dendroica_norm" = "1.00000000, 0.96137731, 0.94146249, 0.90147040, 0.87571248, 0.85918971, 0.83841503, 0.83652212, 0.82247690,0.81928180,0.76344739,0.74228747,0.69769060,0.65145901,0.62322777,0.52965973,0.52706368,0.47598018,0.36544314,0.16740943,0.12848409,0.11224221,0.07126671,0.06929257",
                             ...
                             textInput("file", "Directory where you want to save:", "~/Google Drive/scripts for jobs and data/MCEM.RData"),
...
server <- function(input,output,session) {
  ...
  brts = eventReactive(input$goTree,
                       if(input$brts==0){
  ...
  observe({
    ...
    isolate({ if (rv$run) { 
      ...
      withProgress(message = paste("Performing MCEM iteration", rv$em.iteration), value = 0, {
        ...
        incProgress(rv$MCEM$effective.size[nrow(rv$MCEM)], detail = paste0("Proportion of likelihood: ", rv$MCEM$effective.size[nrow(rv$MCEM)] ))
  ...
  output$lambda <- renderPlot({
    ...
    if(nrow(rv$MCEM)>5){ 
      ...
      plot.lambda = ggplot(MCEM) + geom_line(aes(em.iteration,par1)) + ggtitle(label=paste("Last estimation:  ", lambda.est)) + ylab(expression(lambda)) + xlab("EM iteration")
  ...
  output$mu <- renderPlot({
    if(nrow(rv$MCEM)>5){ 
      ...
      MCEM = rv$MCEM[input$charts:length(rv$MCEM$par3),]
      plot.mu = ggplot(MCEM) + geom_line(aes(em.iteration,par2)) + ggtitle(label=paste("Last estimation:  ",mu.est)) + ylab(expression(mu)) + xlab("EM iteration")
  ...
  output$K <- renderPlot({
    ...
    if(nrow(rv$MCEM)>5){ 
      ...
      MCEM = rv$MCEM[input$charts:length(rv$MCEM$par3),]
      K.plot = ggplot(MCEM) + geom_line(aes(em.iteration,par3)) + ggtitle(label=paste("Last estimation:  ",K.est)) + ylab(expression(K)) + xlab("EM iteration")

...
server <- function(input, output) {
  ...
  DF <- reactive({
    ...
    for(i in 1:nrow(inFile)){
      ...
      PARS = PARS[-nrow(PARS),]
      DF1 = data.frame(iteration=PARS$it,mu0=PARS[,3],lambda0=PARS[,1],N=-PARS[,4],PD=-PARS[,2],E_time=MCEM$E_time,M_time=MCEM$M_time,sample_size=MCEM$sample_size,cores=MCEM$cores,rep=MCEM$rep,comp=MCEM$comp,fhat=MCEM$loglik_hat)

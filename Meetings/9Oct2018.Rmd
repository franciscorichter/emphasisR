...
f.joint1 <- function(tm,lambda,mu,CT=2){
  ...
  for (i in 1:nrow(tm)){
    if (tm[i,1] == CT){
      ...
      if (tm[i,2] == CT){
        ans[i] = dexp(tm[i,1],2*lambda)*(1/2)*exp(-(CT-tm[i,1])*3*mu)
      } else {
...
f.joint <- function(x,lambda,mu,M){
  ...
  for (i in 1:nrow(tm)){
    ...
    tot[tot==2] = 1
    if(sum(tot)>=M){
      stopspec = min(which(cumsum(tot)==M))
  ...
  for (i in 1:n){
    if (!is.na(to[i,2])){
      ...
      if (ta[i,1]==CT){
        ...
        ans[i] = 0.5*dunif(ta[i,1],min=0,max = CT)*dunif(ta[i,2],min=0,max = CT)
      }
    }
  }
  return(ans)
...
g_samp <- function(df,Dim){
  ...
  n = c(2,2+cumsum(to)+cumsum(to-1))
  n = n[tempto==1]
  log_dens = -log(M+1)-Ne*log(ct)-sum(log(Ts)) -sum(log(n))
  return(log_dens)
}


sim.extinct.truncdim <- function(brts,Dim){
  Ne = sample((0:Dim),1)
...
g.sim <- function(x.obs,n=NULL,Dim){
  if (is.vector(x.obs)){
    x.obs <- matrix(rep(x.obs,each=n),ncol=length(x.obs))
  }
  n <- nrow(x.obs)
  brts <- matrix(nrow=n,ncol=2*Dim+1)
  to <- matrix(nrow=n,ncol=2*Dim+1)
  t.ext <- matrix(nrow=n,ncol=2*Dim+1)
  for (i in 1:n){
    wt = no.na(diff(c(0,x.obs[i,])))
    cs = length(wt)-1 # current speciations
    pm = Dim - cs  # possible speciations
    if(pm > 0 ){
      ...
      to[i,1:(length(wt))] = c(rep(1,(length(wt)-1)),2)
...
exp(f.joint(x,lambda = lambda, mu=mu,M=M))
f.joint1(tm,lambda,mu)

x.obs<-observe(x)
x.obs

x.ext<-g.sim(x.obs,Dim=M)
x.ext

g.sim(x.obs[1,],n=6,Dim = M)

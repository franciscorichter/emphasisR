...
lprobto <- function(to,
  ...
  logprob <- sum(to_possible == 1) * log(p) +
    sum(to_possible == 0) * log(1 - p)
  return(logprob)
}

#' @keywords internal
sampletopology <- function(S,
                           p = 0.5) {
  to <- NULL
  if (S > 0) {
    for (i in 1:(2 * S)) {
...
#' @keywords internal
sample_dim_prob <- function(d,
                            max) {
  factorial(2 * d) / sum(factorial(seq(
    from = 0,
    to = 2 * max,
    by = 2
  )))
...
#' @keywords internal
sample.uniform <- function(brts,
                           maxnumspec,
                           single_dimension = NULL) {
  s <- NULL
  ...
  mbts_events <- sim.branchingtimes.and.events(S = s, ct = max(brts), p = 0.5)
  df <- data.frame(
    brts = c(brts, mbts_events$brts),
    to = c(rep(2, length(brts)), mbts_events$to)
  )
  df <- df[order(df$brts), ]
  return(df)
}
...
#' @keywords internal
uniform_tree_augmentation <- function(brts,
                                      maxnumspec) {
  s <- sample(0:maxnumspec, 1)
  mbts_events <- sim.branchingtimes.and.events(S = s, ct = max(brts), p = 0.5)
  df <- data.frame(
...
log.factor.samp.prob <- function(to) {
  top <- utils::head(to, -1)
  number_observed <- c(1, 1 + cumsum(top == 2))
  number_missing <- c(0, cumsum(top == 1) - cumsum(top == 0))
  factor <- -sum(log((2 * number_observed + number_missing)[to == 1])) -
    sum(log(number_missing[to == 0]))
  return(factor)
}

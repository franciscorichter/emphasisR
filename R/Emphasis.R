...
mcem <- function(brts,init_par,n_it=10,MC_ss=100,limit_miss_spec,model="dd"){
  ...
  pars = init_par
  PARS = matrix(ncol = 3,nrow = n_it)
  print(paste(c("starting process, initial parameters",pars)))
  for(i in 1:n_it){
    PARS[i,] = pars = mcem_step(brts,theta_0 = pars,MC_ss = MC_ss,maxnumspec = limit_miss_spec,model = model)
    print(pars)
  }
  return(PARS)

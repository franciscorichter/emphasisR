...
M_step <-function(st,init_par = NULL,model="dd",proportion_of_subset=1){
  ...
  if(proportion_of_subset==1){
    loglik_proportion = 1
    sub_st = lapply(st, "[", weights!=0)
    effective_sample_size = length(st$weights)-sum(weights==0)
  }else{
  ...
  po = subplex(par = init_pars, fn = Q_approx,st = sub_st,model=model,hessian = TRUE)

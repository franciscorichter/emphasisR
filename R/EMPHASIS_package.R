...
emphasis_temp <- function(brts,
  ...
  for (i in 1:1) {
    ...
    load(file = paste(name, "_", input$model, "_",
      as.character(input$sample_size), ".RData",
      sep = ""
    ))
...
emphasis <- function(input,
  ...
  for (i in 1:n_it) {
    ...
    m_step_result <- M_step(st = st, init_par = pars, model = input$model)
    if (!is.infinite(m_step_result$po$value) & !is.na(log(st$fhat))) {
      pars <- m_step_result$po$par
      mcem <- rbind(mcem,
                    data.frame(par1 = pars[1],
                               par2 = pars[2],
                               par3 = pars[3],
                               par4 = pars[4],
                               fhat = log(st$fhat),
                               E_time = st$E_time,
                               M_time = m_step_result$M_time,
                               sample_size = sample_size))
    }
    if (print_process) {
      print(paste("Q random estimation: ", log(m_step_result$po$value)))
...
mcE_step <- function(brts,
  ...
  time <- proc.time()
  if (!parallel) {
    st <- lapply(1:sample_size, function(i) {
      augment_tree_tj(brts = brts, pars = pars, model = model, soc = soc)
    })
  } else {
    st <- parallel::mclapply(1:sample_size, function(i) {
      augment_tree_tj(brts = brts, pars = pars, model = model, soc = soc)
    }, mc.cores = no_cores)
  }
...
M_step <- function(st, init_par, model, reltol = 0.001) {
  ...
  po <- subplex::subplex(par = init_par,
                         fn = Q_approx,
                         st = sub_st,
...
get_contributing_trees <- function(st, exclude_proportion_trees = 0) {
  ...
  sub_st <- list(trees = sub_trees,
                 weights = st$weights[contributing_trees])
  return(sub_st)
}

...
augment_tree <- function(brts, pars, model, soc) {
  ...
  b <- max(brts)
  missing_branches <- data.frame(
    speciation_time = NULL,
    extinction_time = NULL
  )
  cbt <- 0
  while (cbt < b) {
    tree <- data.frame(
      brts = c(
        missing_branches$speciation_time,
        brts,
        missing_branches$extinction_time
      ),
      t_ext = c(
        missing_branches$extinction_time,
        rep(
          Inf,
          length(brts) + nrow(missing_branches)
        )
      ),
      to = c(
        rep(1, nrow(missing_branches)),
        rep(2, length(brts)),
        rep(0, nrow(missing_branches))
      )
    )
    tree <- tree[order(tree$brts), ]
    next_bt <- min(tree$brts[tree$brts > cbt])
    lambda_max <- max(
      sum_speciation_rate(cbt,
        tree,
        pars,
        model,
        soc = soc
    ...
    if (next_speciation_time < next_bt) {
      u2 <- stats::runif(1)
      pt <- sum_speciation_rate(next_speciation_time,
        tree,
        pars,
        model,
        soc = soc
      ) *
        (1 - exp(-mu * (b - next_speciation_time))) / lambda_max
      if (u2 < pt) {
        extinction_time <- next_speciation_time +
          truncdist::rtrunc(1, "exp",
            a = 0, b = (b - next_speciation_time),
            rate = mu
          )
        missing_branches <- rbind(
          missing_branches,
          data.frame(
            speciation_time = next_speciation_time,
  ...
  tree <- data.frame(
    ...
    t_ext = c(
      missing_branches$extinction_time,
      rep(Inf, length(brts) + nrow(missing_branches))
    ),
    to = c(
      rep(1, nrow(missing_branches)),
      rep(2, length(brts)),
...
augment_tree_tj <- function(
  ...
  while (cbt < b) {
    next_bt <- min(tree$brts[tree$brts > cbt])
    l1 <- sum_speciation_rate(cbt, tree, pars, model, soc = soc) *
      (1 - exp(-mu * (b - cbt)))
    l2 <- sum_speciation_rate(next_bt, tree, pars, model, soc = soc) *
      (1 - exp(-mu * (b - next_bt)))
    ...
    if (next_speciation_time < next_bt) { ##
      u2 <- stats::runif(1)
      pt <- sum_speciation_rate(next_speciation_time, tree, pars, model,
        soc = soc
      ) *
        (1 - exp(-mu * (b - next_speciation_time))) / lambda_max
      if (u2 < pt) {
...
lambda_max_rpd5c <- function(cbt,
  ...
  pt <- c(0, tree$pd)[m]
  c2 <- pars[4] * ((n - 1) / n)
  c3 <- exp(-pars[1] * max(tree$brts))
  c4 <- pars[1]
  c1 <- pars[2] + pars[3] * n + pars[4] * ((pt - n * brts_im1) / n)

  bt1 <- cbt
  bt2 <- min(tree$brts[tree$brts > cbt])
  d1 <- -c3 * c4 * exp(c4 * bt1) * (c1 + c2 * bt1) +
         c2 * (-c3) * exp(c4 * bt1) + c2
  d2 <- -c3 * c4 * exp(c4 * bt2) * (c1 + c2 * bt2) +
         c2 * (-c3) * exp(c4 * bt2) + c2
  ...
  if (d1 > 0 & d2 > 0) {
    max_lambda <- sum_speciation_rate(bt2, tree, pars, model, soc = soc) *
      (1 - exp(-mu * (b - bt2)))
  }
  if (d1 < 0 & d2 < 0) {
    max_lambda <- sum_speciation_rate(cbt, tree, pars, model, soc = soc) *
      (1 - exp(-mu * (b - cbt)))
  }
  if ((d1 > 0 & d2 < 0)) {
    max_lambda <- (-c2 - c1 * c4 +
                    c2 * pracma::lambertWp((exp(1 + (c1 * c4) / c2)) / c)) /
                         (c2 * c4)
  ...
  if ((d1 < 0 & d2 > 0)) {
    max_lambda <- max(
      sum_speciation_rate(cbt, tree, pars, model, soc = soc) *
        (1 - exp(-mu * (b - cbt))),

...
sampling_prob <- function(tree, pars, model, numerical = FALSE) {
  if (numerical) {
    ...
    intensity_temp_func <- get(paste0("intensity.", model))
    inte <- intensity_temp_func(tree = tree, pars = pars)
  }
  ...
  if (sum(missing_speciations) > 0) {
    ...
    n_o <- c(soc, soc + cumsum(top == 2))[missing_speciations]
    n_e <- c(0, cumsum(top == 1) - cumsum(top == 0))[missing_speciations]

    brts_miss <- tree$brts[missing_speciations]
    lambda_b <- sapply(brts_miss,
                       speciation_rate,
                       tree = tree,
                       pars = pars,
                       model = model,
...
intensity.rpd1 <- function(tree, pars) {
  ...
  sigma_over_tree <- n * (lambda) *
                      (wt - (exp(-mu * max(tree$brts)) / mu) *
                      (exp(mu * brts_i) - exp(mu * brts_im1)))
...
intensity.numerical <- function(tree, pars, model) {
  nh_rate <- function(x) {
    sum_speciation_rate(x = x,
                        tree = tree,
                        pars = pars,
                        model = model,
                        soc = tree$n[1]) *
                        (1 - exp(-1 * (max(tree$brts) - x) * pars[1]))
  }
  brts_i <- tree$brts
  brts_im1 <- c(0, brts_i[-length(brts_i)])
  inte <- vector(mode = "numeric", length = length(brts_i))
  for (i in seq_along(brts_i)) {
...
intensity.rpd5 <- function(tree, pars) {
  ...
  c1 <- pars[2] +
        pars[3] * n +
        pars[4] * ((pt[-nrow(tree)] -
                    n * c(0, tree$brts[-length(tree$brts)])) / n)
  if (c2 == 0) {
  ...
  for (i in seq_along(brts_im1)) {
    if ((roots[i] > brts_im1[i]) & (roots[i] < brts_i[i])) {
      if ((pars[2] + pars[3] * n[i] + pars[4] * (pt[i] / n[i])) < 0) {
        inte[i] <- (ind.rpd5(x = brts_i[i],
                             c1[i], c2, c3, c4) -
                    ind.rpd5(x = roots[i], c1[i], c2, c3, c4)) * n[i]
      } else {
        inte[i] <- (ind.rpd5(x = roots[i], c1[i], c2, c3, c4) -
                    ind.rpd5(x = brts_im1[i], c1[i], c2, c3, c4)) * n[i]
      }
    } else {
      inte[i] <- (ind.rpd5(x = brts_i[i], c1[i], c2, c3, c4) -
                  ind.rpd5(x = brts_im1[i], c1[i], c2, c3, c4)) * n[i]
    }
  }
  return(inte)
...
ind.rpd5 <- function(x, c1, c2, c3, c4) {
  r <- (c2 * x^2) / 2 +
    c1 * x -
    (c3 * exp(c4 * x) * (c2 * (c4 * x - 1) + c1 * c4)) / (c4^2)
  return(r)
}
...
intensity.rpd5c <- function(tree, pars) {
  ...
  for (i in seq_along(brts_im1)) {
    if ((roots[i] > brts_im1[i]) & (roots[i] < brts_i[i])) {
      if ((pars[2] + pars[3] * n[i] + pars[4] * (pt[i] / n[i])) < 0) {
        inte[i] <- (ind.rpd5(x = brts_i[i], c1[i], c2[i], c3, c4) -
                   ind.rpd5(x = roots[i], c1[i], c2[i], c3, c4)) * n[i]
      } else {
        inte[i] <- (ind.rpd5(x = roots[i], c1[i], c2[i], c3, c4) -
                    ind.rpd5(x = brts_im1[i], c1[i], c2[i], c3, c4)) * n[i]
      }
    } else {
      inte[i] <- (ind.rpd5(x = brts_i[i], c1[i], c2[i], c3, c4) -
                    ind.rpd5(x = brts_im1[i], c1[i], c2[i], c3, c4)) * n[i]

...
Moreover, we define the topology component of a tree at time $t_i$ as the vector $\tau_i \in \{0,1\}^{2*n_{t_i}}$. So, this vector contains a $1$ in the position $j$ if species $j$ has speciated at time $t_i$, or a $1$ in position $(n_{t_i}+j)$ if species $j$ becames extinct at time $t_i$, and $0$ elsewhere. We finally define 
  \[\rho_i= \begin{cases} 
       \lambda_{t_{i},j} & \text{if speciation of species j takes place at time } t_i \\
      \mu_{t_{i},j} & \text{if extinction of species j takes place at time } t_i
   \end{cases}
\]
...
or
 ...
 \begin{equation} \label{lik} \log f_X(x;\phi) =  \sum_{i=1}^{d-1} log(\rho_i)  - \sum_{i=1}^{d} \sigma_i \Delta t_i\end{equation}
...
To compute $g_{X|Y} (x_i|y,\phi^*)$ we use the nonhomogeneous Poisson process characterized by equation (\ref{eq1}). 


Then 

 \begin{equation} \label{sampprob}  g_{X|Y} (x_i|y,\phi^*) = \prod_{i=1}^{d} e^{-\int_{t_i}^{t_i+t_s} n_t \lambda_{z,k}\left(1-e^{-\int_{z}^T\mu_{s,k}ds}\right)dz} \prod_{j=1}^m \lambda_{t_j+t_s,j}\mu_{t_j+t_s,j}e^{-\int_{t_j+t_s}^{t_e}\mu_{t,j}dt} \end{equation}

On the case of spaciation rate piece-wise constant and extinction rate constant, this equation would be equivalent to 

 \begin{equation}
 g_{X|Y}(x|y,\phi) = \prod_{i=1}^d \left(e^{-s_i\left[\Delta t_i+\frac{1}{\mu_0}(e^{-\mu_0 r_i }- e^{-\mu_0(r_i-\Delta t_i)})  \right]} \right) \prod_{i=1}^{m}\left(\lambda_{t^s_i,j} \mu_0  e^{-\mu_0\Delta t^{e}_i} \right) 
\end{equation}


where we consider the subsets of branching times $t^s_i \subset \{t_1,...,t_d\}$ to those branching times corresponding to \emph{speciation of missing species} $i$ and $\Delta t^e_i$ it corresponding extinction waiting time. 
...
\begin{equation} L(y | \theta) = \frac{1}{m}\sum_{i=1}^m \frac{f(x_i,y|\theta)}{g(x_i|y,\theta)} \end{equation}
